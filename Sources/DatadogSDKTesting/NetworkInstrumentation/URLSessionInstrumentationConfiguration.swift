...
struct URLSessionInstrumentationConfiguration {
  internal init(shouldRecordPayload: ((URLSession) -> (Bool)?)? = nil,
  ...
  var injectCustomHeaders: ((inout URLRequest, Span?) -> Void)?
  ...
  var spanCustomization: ((URLRequest, SpanBuilder) -> Void)?

  ///  Called before the span is created, it allows to add extra information to the Span
  var createdRequest: ((URLRequest, Span) -> Void)?
