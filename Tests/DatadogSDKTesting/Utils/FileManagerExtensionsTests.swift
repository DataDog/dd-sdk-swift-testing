...
internal class FileManagerExtensionsTests: XCTestCase {
    ...
    func testExactDirectory_existsCallsBodyOnce() {
        ...
        var visited: [String] = []
        let found = FileManager.default.searchGlob(url) { dir in
    ...
    func testExactDirectory_missingNeverCallsBody() {
        ...
        var called = false
        let found = FileManager.default.searchGlob(url) { _ in called = true; return false }
    ...
    func testWildcard_withTailDoesNotCallBodyOnIntermediateDirs() {
        ...
        FileManager.default.searchGlob(url) { dir in
    ...
    // MARK: - ** wildcard: early exit

    func testWildcard_returnsOnFirstMatch() {
        let url = globRoot.appendingPathComponent("tree/**")
        var count = 0
        let found = FileManager.default.searchGlob(url) { _ in
            count += 1
    ...
    func testWildcard_returnsFalseWhenNothingMatches() {
        let url = globRoot.appendingPathComponent("tree/**/_diag")
        // body always returns false → full traversal, result is false
        let found = FileManager.default.searchGlob(url) { _ in false }
        XCTAssertFalse(found)
    }
    ...
    func testWildcard_customDepthLimitRespected() {
        let url = globRoot.appendingPathComponent("tree/deep/**")
        var visited = Set<String>()
        FileManager.default.searchGlob(url, maxWildcardDepth: 1) { dir in
            visited.insert(dir.lastPathComponent)
            return false
        }
